...
class TrackMateXMLParser:
    ...
    def parse(self) -> List[Track]:
        ...
        # Parse Spots (alle Detections)
        spots_by_id = self._parse_spots()

        # Parse Tracks (welche Spots gehören zusammen)
        self.tracks = self._parse_tracks(spots_by_id)

        return self.tracks

    def _parse_spots(self) -> Dict[int, SpotData]:
        """Parse alle Spots aus dem XML."""
        spots = {}

        # TrackMate XML hat <AllSpots> oder <Model><AllSpots>
        all_spots = self.root.find(".//AllSpots")
        if all_spots is None:
            all_spots = self.root.find(".//Model/AllSpots")

        if all_spots is None:
            raise ValueError("Keine Spots in XML gefunden!")

        # Iteriere über alle <SpotsInFrame>
        for frame_elem in all_spots.findall("SpotsInFrame"):
            frame_num = int(frame_elem.get("frame", 0))

            for spot in frame_elem.findall("Spot"):
                spot_id = int(spot.get("ID"))
                x = float(spot.get("POSITION_X", 0.0))
                y = float(spot.get("POSITION_Y", 0.0))
                z = float(spot.get("POSITION_Z", 0.0))
                t = float(spot.get("POSITION_T", 0.0))

                spots[spot_id] = SpotData(
                    frame=frame_num,
                    x=x,
                    y=y,
                    z=z,
    ...
    def _parse_tracks(self, spots_by_id: Dict[int, SpotData]) -> List[Track]:
        """Parse Tracks aus dem XML."""
        tracks = []

        # TrackMate XML hat <AllTracks> oder <Model><AllTracks>
        all_tracks = self.root.find(".//AllTracks")
        if all_tracks is None:
            all_tracks = self.root.find(".//Model/AllTracks")
        ...
        for track_elem in all_tracks.findall("Track"):
            ...
            track_name = track_elem.get("name", f"Track_{track_id}")

            track = Track(track_id=track_id, name=track_name)

            # Sammle alle Spots in diesem Track
            for edge in track_elem.findall("Edge"):
                source_id = int(edge.get("SPOT_SOURCE_ID"))
                target_id = int(edge.get("SPOT_TARGET_ID"))

                if source_id in spots_by_id and source_id not in [s.frame for s in track.spots]:
                    # Füge Source hinzu (wenn noch nicht da)
                    spot = spots_by_id[source_id]
                    track.spots.append(spot)

                if target_id in spots_by_id:
                    spot = spots_by_id[target_id]
                    track.spots.append(spot)

            # Sortiere Spots nach Frame
            track.spots.sort(key=lambda s: s.frame)
